  ...
  return (
    <div className="p-8 max-w-xl mx-auto">
      <div className="flex items-center gap-2 mb-6">
        <span className="font-mono text-[11px] text-muted-foreground/40">*/activity</span>
      </div>

      <h2 className="font-mono text-base text-foreground mb-6">activity log</h2>
      ...
      <div className="space-y-0">
        {activities.map((a, i) => (
          <div key={i} className="flex items-start gap-3 py-2.5 border-b border-border/30 last:border-0">
            <span className="font-mono text-[12px] text-accent/60 mt-0.5 w-4 text-center shrink-0">{a.icon}</span>
            <div className="flex-1 min-w-0">
              <p className="font-mono text-[12px] text-foreground/70">{a.detail.replace("{username}", username)}</p>
            </div>
            <span className="font-mono text-[10px] text-muted-foreground/40 shrink-0">{a.time}</span>
          </div>
        ))}
      </div>
    </div>
